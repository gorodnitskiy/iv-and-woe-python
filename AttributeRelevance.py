...
class AttributeRelevance:
    ...
    def draw_iv(self, feats, iv, cond):
        ...
        sns.barplot(
            ...
            y='iv',
            data=df,
            palette=self.seq_palette(len(feats)))
    ...
    def draw_woe_multiplot(self, feats, iv, cond):
        ...
        for i in range(n):
            iv_df, iv_value = iv.calculate_iv(feats[i], cond)
            sns.barplot(
                x=feats[i].feature,
                y='woe',
                data=iv_df,
...
class StatsSignificance(Analysis):
    def calculate_chi(self, feat, cond):
        ...
        cramers_v = np.sqrt(chi[0] / n)
        # assume that k=2 (good, bad)
    ...
    def print_chi(self, feat, cond):
        ...
        print('%s is a %s predictor' % (
            feat.feature.capitalize(), self.interpretation(cramers_v)))
...
class IV(Analysis):
    ...
    def draw_woe(self, feat, cond):
        ...
        sns.barplot(
            ...
            y='woe',
            data=iv_df,
            palette=self.seq_palette(len(iv_df.index)))
    ...
    def print_iv(self, feat, cond):
        ...
        print('%s is a %s predictor' % (
            feat.feature.capitalize(), self.interpretation(iv)))
